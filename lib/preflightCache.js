 ...
 * 1. Is the voter registered on-chain?
...
export async function preflightCheck(nullifierHash, electionId) {
  ...
  const voter = await Voter.findOne(
    { nullifierHash, electionId: Number(electionId), status: 'registered' },
    { _id: 1 }
  ).lean();
  ...
  // We check VoteActivity for any vote with this election + look for the
  // nullifier in the blockchain cache. Since votes are anonymized in
  // VoteActivity (no voter address), we use a separate cache approach.
  // For the demo, we check the on-chain hasVoted via a cached flag.
  const existingVote = await VoteActivity.findOne(
    { electionId: Number(electionId), voterNullifier: nullifierHash },
    { _id: 1 }
  ).lean();
