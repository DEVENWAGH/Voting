...
export async function relayRegisterVoter(nullifierHash, metadata = {}) {
  const receipt = await sendRelayTx(() =>
    getContract().registerVoterByRelay(nullifierHash),
  );
  await logRelayTransaction(receipt, 'register_voter', { metadata });
...
export async function relayCastVote(electionId, candidateId, voterNullifier) {
  ...
  await logRelayTransaction(receipt, 'cast_vote', { electionId: Number(electionId) });
...
export async function relayAddCandidate(electionId, name, party, symbol, manifesto, photoUrl, orgSlug) {
  ...
  await logRelayTransaction(receipt, 'add_candidate', { electionId: Number(electionId), orgSlug });
  return { txHash: receipt.hash, blockNumber: receipt.blockNumber };
}
...
/** Check if a nullifier is already registered on-chain (org-wide, not per-election). */
export async function isVoterRegisteredOnChain(nullifierHash) {
  return getReadContract().isRegisteredVoter(nullifierHash);
}
